...
/* harmony default export */ var ProductWrapperBooksvue_type_script_lang_js_ = ({components:{ProductGridItemTwo:()=>Promise.resolve(/* import() */).then(__webpack_require__.bind(null, 56)),QuickView:()=>Promise.resolve(/* import() */).then(__webpack_require__.bind(null, 54))},computed:{newProducts(){return this.$store.getters.getNewProducts.filter(item=>item.category.includes("book"));},bestProducts(){return this.$store.getters.getBestProducts.filter(item=>item.category.includes("book"));},saleProducts(){return this.$store.getters.getSaleProducts.filter(item=>item.category.includes("book"));}}});
